...
private extension LoginReducer {
    ...
    func authenticate(provider: SocialLoginProvider, state: inout State) -> Effect<Action> {
        ...
        return .run { send in
            ...
            case .failure(let error):
                await send(error.isAppleAuthorizationCancelled ? .authenticationCancelled : .authenticationFailed(error.localizedDescription))
            }
        }
    }
    ...
    func restore(_ recovery: AuthWithdrawalRecovery, state: inout State) -> Effect<Action> {
        ...
        return .run { send in
            ...
            case .failure(let error):
                await send(error.isAppleAuthorizationCancelled ? .withdrawalRecoveryRequired(recovery) : .authenticationFailed(error.localizedDescription))
            }
        }
    }
    ...
    func credentialResult(for provider: SocialLoginProvider) async -> Result<String, Error> {
        ...
        case .kakao:
            socialLoginService.isKakaoTalkLoginAvailable
