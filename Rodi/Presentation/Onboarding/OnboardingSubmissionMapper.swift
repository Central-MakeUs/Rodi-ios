...
extension OnboardingState {
    /// 현재 온보딩 선택값을 서버 요청 모델로 변환합니다.
    var memberOnboardingSubmission: MemberOnboardingSubmission? {
        guard let drivingPeriod = licenseDrivingPeriod?.memberDrivingPeriod else {
            return nil
        }

        let baseSubmission = MemberOnboardingSubmission(
            drivingPeriod: drivingPeriod,
            recentFrequency: recentDrivingFrequency?.memberRecentFrequency,
            roadExperiences: selectedRoadDrivingExperiences.map(\.memberRoadExperience),
