...
struct RodiHomeMarkerClusterIndex {
    ...
    static func markers(for items: [RodiCourseItem], zoomLevel: Int) -> [RodiMapMarker] {
        let tier = Tier(zoomLevel: zoomLevel)
        return markers(for: items, tier: tier)
    }

    static func markers(for items: [RodiCourseItem], tier: Tier) -> [RodiMapMarker] {
        guard tier != .individual else {
            return items.compactMap(\.mapMarker)
        }
